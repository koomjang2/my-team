...
import { RANK_COLORS, RANK_LEVEL, BUSINESS_RANKS, RANK_RULES } from '../engine/ranks.js'
import { makeLayout } from './treeLayout.js'
import TreeConnectors from './TreeConnectors.jsx'
import { usePanZoom } from './usePanZoom.js'

const CARD_WIDTH = 92
const layout = makeLayout({ cardWidth: CARD_WIDTH, emptyLaneWidth: 110, branchGap: 40 })

/** 계획 직급과 실질 직급을 비교해 카드 테두리/문구를 정한다 */
function compare(planned, effective) {
  if (planned === 'CSM') return { tone: 'consumer', text: '소비자' }
  const plannedLv = RANK_LEVEL[planned] ?? 0
  const effLv = effective ? RANK_LEVEL[effective] : -1
  if (effLv >= plannedLv) {
    return effLv > plannedLv
      ? { tone: 'over', text: `계획 ${planned} · 실질 ${effective}` }
      : { tone: 'ok', text: `${effective} 달성` }
  }
  return { tone: 'short', text: effective ? `${effective} 까지` : '직급 미달성' }
}

const TONE_STYLE = {
  ok: 'border-emerald-500 bg-emerald-50',
  over: 'border-sky-500 bg-sky-50',
  short: 'border-rose-400 bg-rose-50',
  consumer: 'border-slate-300 bg-slate-50',
}

function EffectiveCard({ node, effRank, gap, onSelect, isSelected }) {
  const cmp = compare(node.rank, effRank)
  const displayRank = node.rank === 'CSM' ? 'CSM' : effRank ?? '—'

  return (
    <div
      data-tree-node="true"
      className={`tree-node-card cursor-pointer rounded-lg border-2 px-1.5 py-1 text-center transition-all
        ${TONE_STYLE[cmp.tone]} ${isSelected ? 'ring-2 ring-blue-500 ring-offset-1' : 'hover:-translate-y-[1px]'}`}
      style={{ width: CARD_WIDTH }}
      onClick={onSelect}
    >
      <div className={`mx-auto inline-block rounded border px-1.5 text-[11px] font-bold ${RANK_COLORS[displayRank] ?? 'border-gray-300 bg-gray-100 text-gray-500'}`}>
        {displayRank}
      </div>
      ...
      <div className={`truncate text-[9px] ${cmp.tone === 'short' ? 'text-rose-600' : 'text-gray-500'}`}>{cmp.text}</div>
      {gap && !gap.achieved && gap.shortfalls.length > 0 && (
...
export default function EffectiveTreePanel({ nodes, effRankMap, gapMap, selectedId, onSelect, rootNode }) {
  ...
  for (const n of nodes) {
    ...
    if (eff && eff !== 'CSM') counts[eff] = (counts[eff] || 0) + 1
  }
      ...
      {rootNode && (
        <div className={`border-b px-3 py-2 text-xs ${rootGap?.achieved ? 'bg-emerald-50' : 'bg-rose-50'}`}>
          <div className="font-semibold text-gray-800">
            {rootNode.name || '나'} · 목표 {rootNode.rank}
            ...
            <span className="ml-2 text-[11px] font-normal text-gray-600">실질 {rootEff ?? '—'}</span>
          </div>
