...
오른쪽 '목표 계보도' 카드의 **회원 ID 는 기본으로 감춰져 있다.** 그 패널 머리줄의
`ID 보이기` 체크박스로 켠다 (`App.jsx` 의 `showEffectiveIds`, `localStorage['my-team-ui-v1']`
에 `menuOpen`·`summaryOpen` 과 함께 저장). 화면을 남에게 보여줄 때 회원번호가
드러나지 않게 하려는 것이라 **없는 값은 꺼짐으로 읽는다**(`=== true`) — 다른 접힘
취향들이 `!== false` 로 켜짐을 기본으로 하는 것과 반대다. 왼쪽 '나의 계보도' 는
이 옵션과 무관하게 늘 보인다.
