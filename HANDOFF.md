...
1. atomy-simul 쪽 사람 한 명에는 my-team 에 없는 정보(하루하루 PV를 얼마나
   ...
   따로 입력할 필요는 없어 보인다.
2. my-team 의 "목표 직급"(이번 보름에 달성하려는 직급)이 atomy-simul 이
   쓰는 직급과 그대로 같은 뜻이라, 이건 그냥 그대로 옮기면 된다. my-team 에만
   있는 "명목 직급"(이름표용, 화면 표시 전용)은 안 옮겨도 된다.
3. my-team 에는 있지만 atomy-simul 은 모르는 유형이 두 가지 있다 —
   **소비자(CSM)** 와 **아직 직급을 안 정한 빈 자리(없음)**. 이 둘을 불러올
   때 통째로 건너뛸지, 아니면 가장 낮은 직급(SSM)으로 쳐서 넣을지를
   **사용자에게 먼저 물어봐야 한다.**
4. 사람을 구분하는 고유 번호(id) 형식이 서로 다르다. 이건 불러오면서
   새로 매겨주면 되니 큰 문제는 아니다.
5. 어느 앱에 "불러오기/내보내기" 버튼을 만들지도 아직 안 정했다 —
   (a) my-team 에 "atomy-simul 로 보내기" 버튼을 만들거나,
   (b) atomy-simul 에 "my-team 파일 불러오기" 버튼을 만들거나 둘 중 하나다.

**다음에 이어서 할 일**: 3번(소비자·빈 자리 처리 방법)과 5번(버튼을 어느
앱에 만들지)을 사용자에게 먼저 확인받은 뒤에 실제 변환 기능을 만든다.
